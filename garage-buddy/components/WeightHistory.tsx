...
export default function WeightHistory({ id, carId, hp, gears, accTime, prediction }: Properties) {

    const { data: session } = useSession()

    const deleteCard = async () => {
        const res = await deleteDoc(doc(db, 'users', session?.user?.email!, 'cars', carId, 'weight predictions', id))
    }
